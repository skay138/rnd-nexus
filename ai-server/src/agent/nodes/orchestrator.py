...
_STRATEGY = """
<strategy>
- 독립 태스크는 같은 라운드에 묶어 병렬 실행
- 한 태스크의 결과(ID·이름 등)를 다음 태스크의 입력으로 쓰는 경우, 반드시 하나의 태스크로 합쳐라 — 워커가 순차 처리함
- [tool_results]를 보고 충분한 데이터가 수집됐으면 tasks=[]로 종료, 부족하면 다른 관점·키워드로 추가 태스크 계획
</strategy>

<task_writing_rules>
- 각 태스크는 독립 실행되므로, 사용자 질문의 핵심 키워드(이름·ID 등)를 반드시 포함하라
- "발견된 ID를 바탕으로", "검색 결과를 토대로", "위 논문", "해당 연구자" 같은 이전 태스크 참조 표현 금지
- 원본 질문 범위를 벗어난 새 도메인·주제로 확장하지 마라

금지 예시 (앞 태스크 결과에 의존 → 독립 실행 불가):
  ✗ ["홍길동 연구원 시맨틱 검색", "발견된 ID로 상세 정보와 네트워크 조회"]
올바른 예시 (하나로 합침 → 워커가 검색→조회 순서를 알아서 처리):
  ✓ ["홍길동 연구원의 상세 프로필과 연구자 네트워크를 조사하라"]
</task_writing_rules>
"""
