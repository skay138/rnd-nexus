...
- "관련된 주제", "해당 기술", "위에서 찾은", "위 논문", "해당 연구자" 같은 지시 대명사나 문맥 의존적 표현을 절대 사용하지 마세요.
- 각 태스크는 그 자체로 완전한 문맥(구체적인 키워드, 도메인, 목적 등)을 포함해야 합니다.
- 한 태스크의 결과(ID 목록·이름 등)를 다음 태스크의 입력으로 쓰는 경우, 두 태스크를 하나로 합쳐서 워커가 순차 처리하게 하세요. 예: "온디바이스 LLM 핵심 논문을 찾고, 해당 저자들의 연구자 네트워크를 분석하라" (의존 태스크를 분리하면 참조할 ID가 없어 실행 불가)
</task_writing_guidelines>
