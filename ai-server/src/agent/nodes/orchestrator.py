...
- 한 태스크의 결과를 다음 태스크의 입력으로 쓰는 경우, 하나의 태스크로 합쳐라 — 워커가 순차 처리함
  (예: "온디바이스 LLM 핵심 논문을 찾고 저자의 연구자 네트워크를 파악하라")
- [tool_results]를 보고 충분한 데이터가 수집됐으면 tasks=[]로 종료, 부족하면 다른 관점·키워드로 추가 태스크 계획
</strategy>

<task_writing_rules>
- 각 태스크는 독립 실행되므로, 사용자 질문의 핵심 키워드를 반드시 포함하라
- "위 논문", "해당 연구자", "관련 기술" 같은 지시대명사·문맥 참조 표현 금지 — ID나 이름을 직접 명시하라
