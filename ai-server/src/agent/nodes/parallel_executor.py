...
async def _run_worker(
    ...
    """
    # OpenAI-compatible (vLLM/Triton): disable Qwen3 thinking for faster tool-call steps
    extra_kwargs: dict = {}
    if settings.llm_provider.lower() == "openai":
        extra_kwargs["extra_body"] = {"chat_template_kwargs": {"enable_thinking": False}}
    llm = get_llm(model=RequestConfig.current().worker_model or settings.rnd_model, **extra_kwargs)
